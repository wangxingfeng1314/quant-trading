...
class TestHighDividendValueStrategy:
    ...
    def test_init_and_schema(self):
        ...
        assert "use_trend_filter" in strat.param_schema
        ...
        assert strat.use_trend_filter == 1
    ...
    def test_buy_signal_above_5_pct_pure_mode(self):
        """测试纯股息模式下股息率 >= 5.0% 触发买入 (use_trend_filter=0)"""
        strat = HighDividendValueStrategy(buy_div_yield=5.0, sell_div_yield=3.5, use_trend_filter=0)
    ...
    def test_trend_filter_suppresses_buy_when_below_ma(self):
        """测试开启趋势增强 (use_trend_filter=1) 时，若价格跌破MA则不买入防左侧被套"""
        strat = HighDividendValueStrategy(buy_div_yield=5.0, sell_div_yield=3.5, use_trend_filter=1, trend_ma_period=20)
    ...
    def test_trend_filter_triggers_buy_when_above_ma(self):
        """测试开启趋势增强 (use_trend_filter=1) 时，价格站上MA顺势确认买入"""
        strat = HighDividendValueStrategy(buy_div_yield=5.0, sell_div_yield=3.5, use_trend_filter=1, trend_ma_period=20)
    ...
    def test_custom_dps(self):
        ...
        strat = HighDividendValueStrategy(buy_div_yield=5.0, sell_div_yield=3.5, use_trend_filter=0, custom_dps=1.0)
